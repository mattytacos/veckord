# Veckord Architecture Specification

## Overview & Architecture Pivot

Veckord is a controller-native Decky plugin for Steam Gaming Mode (target: Bazzite) that controls voice-channel membership, mute/deafen, and voice state in an already-running Vesktop client.
...
- **Primary Architecture Pivot**: **Vesktop / Vencord Local Bridge**. Veckord uses Vesktop as the primary Discord client. A custom Vencord plugin running inside Vesktop exposes a narrow, authenticated local Unix domain socket bridge (`$XDG_RUNTIME_DIR/veckord/bridge.sock`) to Veckord's backend.
...
+------------------------------+------------------------------+
                               | Authenticated Local Unix Domain Socket ($XDG_RUNTIME_DIR/veckord/bridge.sock)
+------------------------------v------------------------------+
| Vesktop Application (Flatpak / Native Host)                |
|  +-------------------------------------------------------+  |
|  | Veckord Vencord Native Server (`native.ts`)             |  |
|  |  - Listens on local Unix socket (Mode 0600)           |  |
|  |  - Enforces allowlist, line-framing & 64KB limits     |  |
|  +---------------------------+---------------------------+  |
|                              | IPC Bridge                   |
|  +---------------------------v---------------------------+  |
|  | Veckord Vencord Renderer (`index.tsx`, `adapter.ts`)   |  |
...
- **Socket Path**: `$XDG_RUNTIME_DIR/veckord/bridge.sock` (Fallback: `/run/user/$UID/veckord/bridge.sock`).
